...
Mat Alg::getHough() {
	vVec4i vec;
	HoughLinesP(canMaskImg, vec, 1, CV_PI / 180, hThresh, minLen, maxGap);
	for(Vec4i v : vec)
		lns.emplace_back(t7ln(v));
	sortHoughLines(*this);
...
void Alg::drawLaneLine(Mat& outMat, t7ln t) {
	...
	if (angleSumsDeq.size() == 9) {
		...
		if (abs(angleAve) > 5)
			drawArrow(outMat, angleAve);
	}
}
void Alg::drawMarks(Mat& outMat) {
	ln(outMat, t7ln(Vec4i(0, yOff, cols, yOff)), pink);
...
void Alg::showImages() {
	...
	cvtColLbl(m, labels);
	Mat vanImg = getVanImg(outFrm, gAve, rAve);
